...
bounding_boxes = [
    ...
    (
        ((1, 2), (1, 5)),
        ((1, 1), (1, 4)),
        fst.def_.decorators.node_list[0].value.value[0],
    ),
...
def test_bounding_box_of_attribute_with_proxy_list():
    assert ((1, 1), (32, 0)) == RED.absolute_bounding_box
    assert ((1, 1), (32, 0)) == RED.class_.absolute_bounding_box
    assert ((2, 5), (5, 7)) == RED.class_.value.get_absolute_bounding_box_of_attribute(
        0
    )
    assert ((6, 5), (9, 17)) == RED.class_.value.get_absolute_bounding_box_of_attribute(
        1
    )
    assert (
        (10, 1),
        (11, 4),
    ) == RED.class_.value.get_absolute_bounding_box_of_attribute(2)
    assert (
        (11, 5),
        (18, 4),
    ) == RED.class_.value.get_absolute_bounding_box_of_attribute(3)
    assert (
        (18, 5),
        (18, 16),
    ) == RED.class_.value.get_absolute_bounding_box_of_attribute(4)
    assert (
        (19, 1),
        (20, 4),
    ) == RED.class_.value.get_absolute_bounding_box_of_attribute(5)
    assert (
        (20, 5),
        (28, 4),
    ) == RED.class_.value.get_absolute_bounding_box_of_attribute(6)
    assert (
        (28, 5),
        (31, 17),
    ) == RED.class_.value.get_absolute_bounding_box_of_attribute(7)
    with pytest.raises(IndexError):
